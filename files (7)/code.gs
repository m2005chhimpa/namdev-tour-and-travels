...
// and any other form submitted via form-handler.js (no OTP step).
...
// Shared by both the OTP-verified path and the generic path, so every
// form's data lands in the sheet the same way regardless of which
// script (enquiry-form.js, otp-form.js, or form-handler.js) sent it.
function buildRow(data) {
  return [
    ...
    data.date || "",
    data.count || data.people || data.travelers || data.days || "",
    data.car || "",
    data.rating || "",
    data.message || ""
  ];
